 ...
 * @author santi
 */
public class UTS_Imass extends AIWithComputationBudget {
    ...
    public UTS_Imass(UnitTypeTable a_utt) {
        super(100,-1);        
        utt = a_utt;
//        System.out.println("SocketAI: constructor 1");
        try {
            connectToServer();
        }catch(Exception e) {
            e.printStackTrace();
    ...
    public UTS_Imass(int mt, int mi, String a_sa, int a_port, int a_language, UnitTypeTable a_utt) {
        super(mt, mi);
        serverAddress = a_sa;
//        System.out.println("SocketAI: constructor2");
    ...
    private UTS_Imass(int mt, int mi, UnitTypeTable a_utt, int a_language, Socket socket) {
        super(mt, mi);
//        System.out.println("SocketAI: constructor3");
        communication_language = a_language;
        utt = a_utt;
        try {
            this.socket = socket;
     ...
     */
    public static UTS_Imass createFromExistingSocket(int mt, int mi, UnitTypeTable a_utt, int a_language, Socket socket) {
//        System.out.println("SocketAI: constructor4");
        return new UTS_Imass(mt, mi, a_utt, a_language, socket);
    }
    ...
    public void reset() {
        try {
            ...
            // Used for remote call to signal its a dummy agent
//             out_pipe.append("slave\n");
//            out_pipe.flush();
//            in_pipe.readLine();           
    ...
    public PlayerAction getAction(int player, GameState gs) throws Exception {
        // send the game state:
//        System.out.println("SocketAI: get actions");
...
//            }
            ...
            while(!in_pipe.ready() && System.currentTimeMillis()-start < TIME_BUDGET*3);
            if (System.currentTimeMillis()-start > TIME_BUDGET)
                System.out.println("took to long: " + Long.toString(System.currentTimeMillis()-start));
            // parse the action:
            String actionString = in_pipe.readLine();
...
//        System.out.println("post game ");
    ...
    @Override
    public AI clone() {
        return new UTS_Imass(TIME_BUDGET, ITERATIONS_BUDGET, serverAddress, serverPort, communication_language, utt);
    }
